...
|----------|--------|-----------|-------|----------|
| **Итерация 0** | ⏳ Ожидает | 0 | 2 | 0% |
| **Итерация 1** | 🔄 В процессе | 0 | 8 | 0% |
| **Итерация 2** | ⏳ Ожидает | 0 | 6 | 0% |
| **Итерация 3** | ⏳ Ожидает | 0 | 5 | 0% |
| **Итерация 4** | ⏳ Ожидает | 0 | 4 | 0% |
| **Итерация 5** | ⏳ Ожидает | 0 | 5 | 0% |

**Общий прогресс:** 0/30 задач (0%)

---

## 🌿 Итерация 0: Подготовка рабочей среды

**Цель:** Создать изолированную ветку для разработки функции экспорта спектрограммы

### Задачи:

- [ ] **0.1** Создать и переключиться на ветку `dev_spectrogram_export`
  ```bash
  git checkout -b dev_spectrogram_export
  ```
- [ ] **0.2** Убедиться, что базовая ветка `main` актуальна
  ```bash
  git checkout main
  git pull origin main
  git checkout dev_spectrogram_export
  ```
...
- [ ] **2.3** Реализовать метод exportToNPY согласно [@Technical_Specification.md#2-требования-к-функциональности](Technical_Specification.md#2-требования-к-функциональности)
  - [ ] Использование libnpy для сохранения сырых данных
  - [ ] Правильная структура данных (height, width)
  - [ ] Обработка ошибок записи NPY файлов

- [ ] **2.4** Реализовать метод exportToTIFF согласно [@Technical_Specification.md#2-требования-к-функциональности](Technical_Specification.md#2-требования-к-функциональности)
  - [ ] Использование tinytiff для сохранения нормализованных данных
  - [ ] Корректное отображение спектрограммы
  - [ ] Обработка ошибок записи TIFF файлов
...
- [ ] **3.3** Добавить вызовы экспорта в процесс обработки согласно [@Design_Document.md#5-интеграция-с-существующей-системой](Design_Document.md#5-интеграция-с-существующей-системой)
  - [ ] Вызов exportToNPY() в VD_VoiceDetector::processFrame()
  - [ ] Вызов exportToTIFF() в VD_VoiceDetector::processFrame()
  - [ ] Проверка флага m_export_data
  - [ ] Генерация общего timestamp для обоих форматов
...
## 🔀 Итерация 5: Слияние с основной веткой

**Цель:** Интегрировать готовую функциональность в основную ветку проекта

### Задачи:

- [ ] **5.1** Завершить все задачи и тесты в ветке `dev_spectrogram_export`
- [ ] **5.2** Создать Pull Request для слияния с `main`
- [ ] **5.3** Провести code review и получить одобрение
- [ ] **5.4** Выполнить слияние в `main` ветку
- [ ] **5.5** Удалить рабочую ветку `dev_spectrogram_export`
